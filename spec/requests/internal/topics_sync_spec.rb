...
RSpec.describe 'Internal Topics sync', type: :request do
  ...
  it 'rejects requests without the admin token' do
    post '/internal/topics/sync', params: { topics: [] }
  ...
  it 'updates existing topics' do
    topic_id = build_topic_id
    Topic.create!(id: topic_id, name: 'Old Name')

    post '/internal/topics/sync',
      params: { topics: [{ id: topic_id, name: 'New Name' }] },
      headers: headers
  ...
  it 'deletes topics by id' do
    topic_id = build_topic_id
    Topic.create!(id: topic_id, name: 'Delete Me')

    post '/internal/topics/sync',
      params: { delete_ids: [topic_id] },
      headers: headers
